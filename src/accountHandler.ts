...
export const updatePasswordHandler = async (c: any) => {
    const db = drizzle(c.env.DB)
...
export const deleteHandler = (app: Hono<{ Bindings: { DB: any } }>) => {
  app.delete('/account/delete/:id', async (c) => {
    const db = drizzle(c.env.DB)
    ...
    // So sánh username trong token với tài khoản cần xóa (chỉ jobseeker được phép xóa tài khoản của mình)
